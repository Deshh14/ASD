...
private:
    ...
    Node<Tkey, Tval>* get_head_for_level(size_t level) const {
        auto it = heads.begin();
        for (size_t i = 0; i < level && it != heads.end(); ++i) {
            ++it;
        }
        return (it != heads.end()) ? *it : nullptr;
    }
...
SkipList<Tkey, Tval>::SkipList(size_t max_levels)
    : lvl(0), max_lvl(max_levels), count(0) {
    srand(time(nullptr));

    Tkey dummy_key = Tkey();
    Tval dummy_val = Tval();
    for (size_t i = 0; i < max_lvl; ++i) {
        Node<Tkey, Tval>* head = new Node<Tkey, Tval>(dummy_key, dummy_val, max_lvl);
...
void SkipList<Tkey, Tval>::insert(const Tkey& key, const Tval& val) {
    ...
    Node<Tkey, Tval>* update[max_lvl];
    for (size_t i = 0; i < max_lvl; ++i) {
        update[i] = nullptr;
    }

    Node<Tkey, Tval>* current = get_head_for_level(lvl - 1);
...
void SkipList<Tkey, Tval>::erase(const Tkey& key) {
    ...
    if (current && current->data.first == key) {
        ...
        while (lvl > 1 && get_head_for_level(lvl - 1)->next[lvl - 1] == nullptr) {
            lvl--;
        }
    }
    else {
...
void SkipList<Tkey, Tval>::print(std::ostream& out) const {
    ...
    Node<Tkey, Tval>* current = get_head_for_level(0)->next[0];
    int index = 0;
    while (current) {
